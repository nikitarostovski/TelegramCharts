...
class MainViewController: UITableViewController {
    ...
    let dates: [TimeInterval] = [1542412800000,
                           1542499200000,
                           1542585600000,
                           1542672000000,
                           1542758400000,
    ...
    let testLineB: [Int64] = [22,
                              ...
                              73,
                              52]
    private var axisData: AxisData? {
        let dateStrings: [String] = dates.map { Date(timeIntervalSince1970: $0).stringValue() }
        return AxisData(xTitles: dateStrings)
    }
    private var chartsData: ChartsData? {
        let lineA = (testLineA, UIColor(hexString: "4bd964"))
        let lineB = (testLineB, UIColor(hexString: "fe3c30"))
        return ChartsData(lines: [lineA, lineB])
    }
    private var gridData: GridData? {
        let allValues = testLineA + testLineB
        if let max = allValues.max() {
            return GridData(maxY: max)
        }
        return nil
    }
